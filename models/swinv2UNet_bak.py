...
class unet_swin(nn.Module):
    def __init__(self, config, size="swinv2_tiny_window16_256", img_size=512):
        ...
        self.segmentation_head = SegmentationHead(
            ...
            out_channels=config.num_classes+1,  # todo 三类别
            kernel_size=3,
            upsampling=4
        )
    ...
    def forward(self, x, domain_idx):
        ...
        try:
            ...
            # 获取特征，无论是训练还是验证
            kv_features = self.encoder.extract_domain_kv(x2)

            # 添加调试信息
            print(f"[DEBUG] Processing batch with domain_idx: {domain_idx}")

            # 分别处理每个样本的域索引
            for i in range(len(domain_idx)):
                idx = domain_idx[i].item()
                sample_features = kv_features[:, i:i+1]

                print(f"[DEBUG] Sample {i} - Domain {idx}")
                print(f"[DEBUG] Memory bank size for domain{idx}: {len(self.domain_banks.banks[f'domain{idx}'].memory)}")
                print(f"[DEBUG] Sample features shape: {sample_features.shape}")

                if self.training:
                    print(f"[DEBUG] Training mode")
                    print(f"[DEBUG] Interaction score: {interaction_score:.4f}")
                    print(f"[DEBUG] x1_score: {x1_score:.4f}")

                    updated = self.domain_banks.banks[f'domain{idx}'].update(
                        sample_features,
                        interaction_score
                    )
                    print(f"[DEBUG] Memory bank updated: {updated}")

                    if updated:
                        print("[DEBUG] Computing losses...")
                        curr_memory_loss = self.domain_banks.compute_loss(sample_features, idx)
                        curr_consistency_loss = F.mse_loss(masks, x1_output)
                        print(f"[DEBUG] Memory loss: {curr_memory_loss:.4f}")
                        print(f"[DEBUG] Consistency loss: {curr_consistency_loss:.4f}")
            ...
            with torch.no_grad():
                if decoder_output1.requires_grad:
                    grad_norm = torch.norm(decoder_output1.grad) if decoder_output1.grad is not None else 0
                    if grad_norm > 1.0:
                        print(f"[WARNING] Large gradient in decoder_output1: {grad_norm}")

                if x1_output.requires_grad:
                    grad_norm = torch.norm(x1_output.grad) if x1_output.grad is not None else 0
                    if grad_norm > 1.0:
