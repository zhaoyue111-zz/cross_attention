        ...
        if self.mode == 'train':
            ...
            for main_domain in self.train_domains:
                ...
                for main_idx in main_indices:
                    other_domains = [d for d in self.train_domains if d != main_domain]
            ...
            for test_domain in self.test_domains:
                img_paths = glob.glob(os.path.join(cfg.data_dir, f'{test_domain}/JPEGImages', '*.jpg'))
                img_paths.sort(key=lambda x: int(os.path.basename(x).split('_')[1].split('.')[0]))
            ...
            for other_domain in self.train_domains:
                ...
                img_paths = glob.glob(os.path.join(cfg.data_dir, f'{other_domain}/JPEGImages', '*.jpg'))
                img_paths.sort(key=lambda x: int(os.path.basename(x).split('_')[1].split('.')[0]))
