...
class PairDataset(Dataset):
    def __init__(self, cfg=None, transforms=None, mode='train', is_train_split=True, train_domains=None, test_domains=None):
        ...
        Args:
            ...
            if train：训练集使用train中 验证集使用test中
        """
        self.mode = mode
        self.cfg = cfg
        ...
        if self.mode == 'train':
            ...
            for domain in self.train_domains:
                train_img_paths = glob.glob(os.path.join(self.cfg.data_dir, domain, 'train', 'image', '*.jpg'))    # 训练集图片路径
                train_mask_paths = glob.glob(os.path.join(self.cfg.data_dir, domain, 'train', 'masks', '*.png'))   # TODO:mask采用极值化后?
                ...
                val_img_paths = glob.glob(os.path.join(self.cfg.data_dir, domain, 'test', 'image', '*.jpg'))    # 验证集图片路径
            ...
            for test_domain in self.test_domains:
                img_paths = glob.glob(os.path.join(self.cfg.data_dir, test_domain, 'test', 'image', '*.jpg'))    # 测试集图片路径
            ...
            for other_domain in self.train_domains:
                ...
                img_paths = glob.glob(os.path.join(self.cfg.data_dir, other_domain, 'train', 'image', '*.jpg'))    # 训练集图片路径
                img_paths.sort(key=lambda x: os.path.splitext(os.path.basename(x))[0])
    ...
    def __getitem__(self, idx):
        if self.mode == 'train':
            if self.is_train_split:
                ...
                img1_path = os.path.join(self.cfg.data_dir, main_domain, 'train', 'image', img1_name)
                ...
                img2_path = os.path.join(self.cfg.data_dir, other_domain,'train', 'image', img2_name)
            else:
                main_domain, main_idx, other_domain = self.pairs[idx]
                img1_name = os.path.join(os.path.basename(main_idx) + '.jpg')     # 图片名称
                img1_path = os.path.join(self.cfg.data_dir, main_domain, 'test', 'image', img1_name)
                label1_name = os.path.join(os.path.basename(main_idx) + '.png')
                ...
                random_idx = random.choice(available_indices)
                img2_name = os.path.join(os.path.basename(random_idx) + '.jpg')   # 图片名称
                img2_path = os.path.join(self.cfg.data_dir, other_domain, 'test', 'image', img2_name)
            ...
            # 将域名转换为数字索引
            domain_idx = torch.tensor(self.domain_to_idx[other_domain])

            return img1, img2, label1, domain_idx
        else:
