...
def train_one_epoch(model, optimizer, scheduler, dataloader, device, epoch, cfg):
    ...
    for step, ((images1, masks1, path1), (images2, masks2, path2)) in pbar:
        images = torch.cat((images1, images2), dim=0).to(device, dtype=torch.float)
        masks = torch.cat((masks1, masks2), dim=0).to(device, dtype=torch.float)

        print(f"Reading images from: {path1}, {path2}")
