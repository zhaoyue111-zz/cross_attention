...
import os
from utils.tsne import visualize_sample_features
...
def train_epoch(model, train_loader, optimizer, criterion, device, epoch, args):
    ...
    for i, batch_data in enumerate(pbar):
        ...
        with autocast():
            ...
            # 4. 总损失计算：添加梯度裁剪  todo_去掉了main_loss
            loss = single_loss + consistency_loss + memory_loss
...
def save_checkpoint(state, save_dir):
    ...
    memory_state = {'memory_banks': memory_banks}
    torch.save(memory_state, memory_path)
    print(f"Memory banks saved to {memory_path}")
...
def train(args):
    ...
    for epoch in range(start_epoch, args.n_iter + 1):
        ...
        # 定期保存检查点
        save_dir = os.path.join(args.checkpoint_dir, f'checkpoint_epoch{epoch:03d}')
        if epoch % 50 == 0:
            save_checkpoint(
                state=checkpoint,
                save_dir=save_dir
        ...
        # 如果是最佳模型，保存最佳检查点
        save_dir2 = os.path.join(args.checkpoint_dir, 'model_best')
    ...
    # 绘制tsne图
    visualize_sample_features(model, train_loader, args.log_dir, device)
...
def load_checkpoint(model, checkpoint_path, device):
    ...
    # 修改路径匹配
    model_path = os.path.join(checkpoint_path, 'model.pth')  # 改为model.pth
    if os.path.exists(model_path):
        checkpoint = torch.load(model_path)
        model.load_state_dict(checkpoint['state_dict'])  # 改为state_dict
